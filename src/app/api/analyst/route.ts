  ...
  try {
    ...
    if (!body.food_register_id) {
      return NextResponse.json(
        { error: "food_register_id is required" },
        { status: 400 },
      );
    }
    ...
    if ((existing as any[]).length > 0) {
      // Update
      const fields = Object.keys(dataToSave).filter(
        (k) => k !== "food_register_id",
      );
...
export async function DELETE(req: Request) {
  try {
    ...
    await db.query("DELETE FROM lab_analysis WHERE food_register_id = ?", [
      body.food_register_id,
    ]);
