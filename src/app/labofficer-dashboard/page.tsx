...
export default function LabOfficerDashboard() {
    ...
    const [editForm, setEditForm] = useState({
        sample_date: "", collection_time: "", reference_number: "",
        sample_type: "", phi_area: "", moh_area: "", analysis_details: "", handed_over_by: "", analyzed_by: "",
    });
    ...
    const emptyForm = { sample_date: "", collection_time: "", reference_number: "", sample_type: "", phi_area: "", moh_area: "", analysis_details: "", handed_over_by: "", analyzed_by: "" };
    ...
    const fetchRows = async (q = "") => {
        setIsLoading(true);
        try {
            const res = await fetch(`/api/labofficer?search=${encodeURIComponent(q)}`);
            const data = await res.json();
            if (Array.isArray(data)) setRows(data);
            else showMessage("Error loading data: " + (data.error || "Unknown error"));
        } catch {
    ...
    const createRow = async () => {
        const res = await fetch("/api/labofficer", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(newForm) });
        const data = await res.json();
        showMessage(data.message || data.error);
        if (!data.error) { setNewForm(emptyForm); fetchRows(search); }
    };

    const startEdit = (row: FoodRegister) => {
        setEditingId(row.id);
        setEditForm({ sample_date: row.sample_date || "", collection_time: row.collection_time || "", reference_number: row.reference_number || "", sample_type: row.sample_type || "", phi_area: row.phi_area || "", moh_area: row.moh_area || "", analysis_details: row.analysis_details || "", handed_over_by: row.handed_over_by || "", analyzed_by: row.analyzed_by || "" });
    };

    const saveEdit = async () => {
        if (!editingId) return;
        const res = await fetch("/api/labofficer", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: editingId, ...editForm }) });
        const data = await res.json();
        showMessage(data.message || data.error);
        setEditingId(null);
        fetchRows(search);
    ...
    const deleteRow = async (id: number) => {
        ...
        const res = await fetch("/api/labofficer", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
        const data = await res.json();
        showMessage(data.message || data.error);
        fetchRows(search);
    };
...
<style>* { box-sizing: border-box; margin: 0; padding: 0; } @page { size: A4 portrait; margin: 0; } html, body { width: 210mm; font-family: Arial, sans-serif; font-size: 11px; color: #000; background: #fff; } .page { width: 210mm; height: 297mm; padding: 10mm 15mm; display: flex; flex-direction: column; overflow: hidden; page-break-after: always; } .header { display: grid; grid-template-columns: 80px 1fr 80px; align-items: center; border: 1px solid #000; padding: 10px; } .header .logo img { width: 65px; height: 65px; object-fit: contain; } .header .title-block { text-align: center; } .header .title-block h1 { font-size: 18px; font-weight: bold; text-transform: uppercase; margin: 0; } .header .title-block .sinhala { font-size: 18px; font-weight: bold; margin-top: 4px; } table { width: 100%; border-collapse: collapse; table-layout: fixed; } td { border: 1px solid #000; padding: 4px 10px; vertical-align: middle; } .doc-table td { height: 24px; } .sample-num-row { display: flex; border: 1px solid #000; border-top: none; } .sample-num-label, .sample-num-value { font-size: 15px; font-weight: bold; padding: 12px 15px; display: flex; align-items: center; } .sample-num-label { width: 35%; border-right: 1px solid #000; } .sample-num-value { flex: 1; } .detail-table td { height: 36px; font-size: 12px; } .two-col { display: flex; border-left: 1px solid #000; border-right: 1px solid #000; flex: 1; min-height: 200px; } .col-left { width: 50%; border-right: 1px solid #000; padding: 10px; display: flex; flex-direction: column; } .col-right { width: 50%; padding: 10px; } .col-header { font-weight: bold; margin: -10px -10px 8px -10px; padding: 10px; border-bottom: 1px solid #000; } .remarks { margin-top: auto; font-weight: bold; } .bottom-table td { height: 42px; font-size: 12px; } .footer { margin-top: auto; padding: 5px 0; font-size: 9px; text-align: center; border-top: 1px solid #ccc; color: #555; } .page { position: relative; } .watermark { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) rotate(-45deg); font-size: 55px; color: rgba(0, 0, 0, 0.08); white-space: nowrap; pointer-events: none; z-index: 10; font-weight: bold; }</style>
    ...
    const generateSummaryReportHtml = (selectedRows: FoodRegister[], insertDate: boolean = true, title: string = "Summary Report of Selected Food Samples") => {
...
<style>
    ...
    const handleDateRangeReport = () => {
        if (!dateRange.start || !dateRange.end) return alert("Please select both start and end dates");
        const filtered = rows.filter(r => {
            const date = new Date(r.sample_date);
            return date >= new Date(dateRange.start) && date <= new Date(dateRange.end);
        });

        if (filtered.length === 0) return alert("No records found in this date range");
        ...
        <div>
            ...
            <div className="topbar">
                <div className="topbar-brand">
                    ...
                    <span style={{ color: "var(--primary)", fontWeight: 800 }}>Lab Officer</span>
                    <span>Dashboard</span>
                </div>
            ...
            <div className="page-wrapper">
                ...
                <div className="card" style={{ marginBottom: "1.5rem" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1rem" }}>
                        <div><label className="form-label">Sample Date</label><input type="date" className="form-input" value={newForm.sample_date} onChange={e => setNewForm({ ...newForm, sample_date: e.target.value })} /></div>
                        <div><label className="form-label">Collection Time</label><input type="time" className="form-input" value={newForm.collection_time} onChange={e => setNewForm({ ...newForm, collection_time: e.target.value })} /></div>
                        <div><label className="form-label">Reference Number</label><input className="form-input" placeholder="e.g. REF-001" value={newForm.reference_number} onChange={e => setNewForm({ ...newForm, reference_number: e.target.value })} /></div>
                        <div>
                            <label className="form-label">Sample Type</label>
                            <select className="form-select" value={newForm.sample_type} onChange={e => setNewForm({ ...newForm, sample_type: e.target.value })}>
                        ...
                        <div><label className="form-label">PHI Area</label><input className="form-input" placeholder="PHI Area" value={newForm.phi_area} onChange={e => setNewForm({ ...newForm, phi_area: e.target.value })} /></div>
                        <div><label className="form-label">MOH Area</label><input className="form-input" placeholder="MOH Area" value={newForm.moh_area} onChange={e => setNewForm({ ...newForm, moh_area: e.target.value })} /></div>
                        <div><label className="form-label">Handed Over By</label><input className="form-input" placeholder="Name" value={newForm.handed_over_by} onChange={e => setNewForm({ ...newForm, handed_over_by: e.target.value })} /></div>
                        <div><label className="form-label">Assign Analyst</label><input className="form-input" placeholder="Analyst Name" value={newForm.analyzed_by} onChange={e => setNewForm({ ...newForm, analyzed_by: e.target.value })} /></div>
                        <div><label className="form-label">Analysis Details</label><input className="form-input" placeholder="Details" value={newForm.analysis_details} onChange={e => setNewForm({ ...newForm, analysis_details: e.target.value })} /></div>
                    </div>
                    <div style={{ marginTop: "1.25rem", display: "flex", justifyContent: "flex-end" }}>
                        <button onClick={createRow} className="btn btn-primary">✓ Register Sample</button>
                    </div>
                </div>

                {/* ── Bulk Actions & Date Range Reporting ── */}
                <div style={{ display: "flex", flexWrap: "wrap", alignItems: "flex-end", gap: "1.25rem", marginBottom: "1.5rem", background: "#f8fafc", padding: "1.5rem", borderRadius: "0.75rem", border: "1px dashed var(--border)" }}>
                    <div style={{ flex: 1, minWidth: "300px" }}>
                        <div style={{ marginBottom: "0.75rem", fontSize: "0.875rem", fontWeight: 700, color: "var(--text)" }}>📜 Date Range Summary Report</div>
                        <div style={{ display: "flex", gap: "0.75rem" }}>
                            ...
                            <button onClick={handleDateRangeReport} className="btn btn-primary" style={{ height: "42px", marginTop: "1.6rem" }}>
                                📑 Generate Report
                            </button>
                        </div>
                    </div>

                    <div style={{ borderLeft: "1px solid #e2e8f0", paddingLeft: "1.25rem", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                        <div style={{ fontSize: "0.875rem", fontWeight: 700, color: "var(--text)" }}>📦 Bulk Actions ({selectedIds.length} selected)</div>
                        <div style={{ display: "flex", gap: "0.625rem", flexWrap: "wrap" }}>
                            <button onClick={handleBulkPrint} disabled={selectedIds.length === 0} className="btn btn-outline-primary">
                                🖨️ Print Selected Sheets
                            </button>
                            <button onClick={() => handleBulkSummaryReport(true)} disabled={selectedIds.length === 0} className="btn btn-outline-primary">
                                📋 Generate Summary Form
                            </button>
                            {selectedIds.length > 0 && <button onClick={() => setSelectedIds([])} className="btn btn-ghost">Deselect All</button>}
                        </div>
                    </div>
                </div>
                ...
                <div className="card" style={{ padding: 0, overflow: "hidden" }}>
                    {isLoading ? (
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "4rem", gap: "0.75rem", color: "var(--muted)" }}>
                            <svg style={{ width: "1.25rem", height: "1.25rem", animation: "spin 1s linear infinite" }} viewBox="0 0 24 24" fill="none">
                        ...
                        <div style={{ overflowX: "auto", transform: "rotateX(180deg)" }}>
                            <table className="data-table" style={{ transform: "rotateX(180deg)" }}>
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    {rows.map(row => (
                                        <tr key={row.id} style={{ background: selectedIds.includes(row.id) ? "#f1f5f9" : "transparent" }}>
                                            <td style={{ paddingLeft: "1.25rem" }}>
                                                <input type="checkbox" checked={selectedIds.includes(row.id)} onChange={() => toggleSelection(row.id)} />
                                            </td>
                                            <td style={{ color: "var(--muted)", fontWeight: 600 }}>{row.id}</td>
                                            <td>{editingId === row.id ? <input type="date" value={editForm.sample_date.slice(0, 10)} onChange={e => setEditForm({ ...editForm, sample_date: e.target.value })} /> : formatDateLong(row.sample_date)}</td>
                                            <td>{editingId === row.id ? <input type="time" value={editForm.collection_time} onChange={e => setEditForm({ ...editForm, collection_time: e.target.value })} /> : <span style={{ fontFamily: "monospace", fontSize: "0.8rem" }}>{row.collection_time || "—"}</span>}</td>
                                            <td><span style={{ fontWeight: 600 }}>{row.sample_number}</span></td>
                                            <td>{editingId === row.id ? <input value={editForm.reference_number} onChange={e => setEditForm({ ...editForm, reference_number: e.target.value })} /> : row.reference_number}</td>
                                            <td>
                                                {editingId === row.id ? (
                                                    <select value={editForm.sample_type} onChange={e => setEditForm({ ...editForm, sample_type: e.target.value })}>
                                                        <option value="Water">Water</option>
                                                        <option value="Spices">Spices</option>
                                                        <option value="Salt">Salt</option>
                                                    </select>
                                                ) : <span className={TYPE_BADGE[row.sample_type] || "badge badge-primary"}>{row.sample_type}</span>}
                                            </td>
                                            <td title={row.phi_area}>{editingId === row.id ? <input value={editForm.phi_area} onChange={e => setEditForm({ ...editForm, phi_area: e.target.value })} /> : row.phi_area}</td>
                                            <td title={row.moh_area}>{editingId === row.id ? <input value={editForm.moh_area} onChange={e => setEditForm({ ...editForm, moh_area: e.target.value })} /> : row.moh_area}</td>
                                            <td title={row.analyzed_by || "No Analyst Assigned"}>
                                                {editingId === row.id ? (
                                                    <input value={editForm.analyzed_by} onChange={e => setEditForm({ ...editForm, analyzed_by: e.target.value })} />
                                                ) : <span style={{ fontWeight: 600, color: "var(--text)" }}>{row.analyzed_by || "—"}</span>}
                                            </td>
                                            <td title={row.handed_over_by}>
                                                {editingId === row.id ? <input value={editForm.handed_over_by} onChange={e => setEditForm({ ...editForm, handed_over_by: e.target.value })} /> : row.handed_over_by}
                                            </td>
                                            <td style={{ textAlign: "right", paddingRight: "1.25rem", whiteSpace: "nowrap" }}>
                                                <div style={{ display: "flex", gap: "0.375rem", justifyContent: "flex-end", flexWrap: "nowrap" }}>
                                                    {editingId === row.id ? (
                                                        <>
                                                            <button onClick={saveEdit} className="btn btn-success btn-sm">✓ Save</button>
                                                            <button onClick={() => setEditingId(null)} className="btn btn-ghost btn-sm">Cancel</button>
                                                        </>
                                                    ) : (
                                                        <>
                                                            <button onClick={() => startEdit(row)} className="btn btn-warn btn-sm">✏️ Edit</button>
                                                            <button onClick={() => deleteRow(row.id)} className="btn btn-danger btn-sm">🗑</button>
                                                            <button onClick={() => handlePrint(row)} className="btn btn-outline-primary btn-sm" title="Print Work Sheet">🖨️</button>
                                                            <button onClick={() => handleSingleSummaryReport(row)} className="btn btn-outline-primary btn-sm" title="Print Summary Info">📋</button>
                                                        </>
                                                    )}
                                                </div>
                                            </td>
                                    ...
                                    {rows.length === 0 && (
                                        <tr><td colSpan={12} style={{ textAlign: "center", padding: "3rem", color: "var(--muted)" }}>No records found. Register a sample above.</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
