...
export default function LabAnalystDashboard() {
    ...
    useEffect(() => {
        if (sampleType === "Spices") {
            setPrintSettings({ revisionNo: "03", issueNo: "01", dateOfRevision: "01/01/2024", dateOfIssue: "23/01/2019" });
        } else if (sampleType === "Water" || sampleType === "Salt") {
            setPrintSettings({ revisionNo: "01", issueNo: "01", dateOfRevision: "", dateOfIssue: "" });
        }
    }, [sampleType]);
        ...
        if (searchSample.trim().length > 0) {
            const t = setTimeout(() => {
                fetch(`/api/analyst?search=${encodeURIComponent(searchSample)}`)
                    .then(r => r.json()).then(d => setSamples(d));
            }, 300);
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            ...
            const data = await res.json();
            showMessage(data.message || data.error);
            if (Number(selectedSampleId) === id) { setSelectedSampleId(""); setForm(initialForm); }
            fetchSavedList();
        } catch {
    ...
    const handlePrint = () => {
        ...
        const mII = calcMoisture(form.moist_m1_ii, form.moist_m2_ii, form.moist_m3_ii);
        const mAvg = (mI + mII) / 2;
        const aI = calcAsh(form.ash_m1_i, form.ash_m2_i, form.ash_m3_i);
        const aII = calcAsh(form.ash_m1_ii, form.ash_m2_ii, form.ash_m3_ii);
        const aAvg = (aI + aII) / 2;
        const acI = calcAcidAsh(form.acid_m2_i, form.acid_m1_i, form.ash_m2_i - form.ash_m1_i);
        const acII = calcAcidAsh(form.acid_m2_ii, form.acid_m1_ii, form.ash_m2_ii - form.ash_m1_ii);
...
<script>window.onload=()=>{setTimeout(()=>{window.print();window.close();},700);}</script>
    ...
    const handlePrintFullReport = () => {
        const origin = typeof window !== "undefined" ? window.location.origin : "";
        const currentDateTime = new Date().toLocaleString("en-GB", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: true });
        let displaySampleNo = form.sample_number.startsWith("PFLF/") ? form.sample_number : "PFLF/" + form.sample_number;
        displaySampleNo = displaySampleNo.replace(/^(PFLF\/[a-zA-Z]+)(\d+.*)$/, "$1/$2");
...
<script>window.onload=()=>{setTimeout(()=>{window.print();window.close();},700);}</script>
    ...
    const ROTextarea = ({ value }: { value: string }) => (
        <div style={{
            ...
            color: "#1e293b", minHeight: "8rem", lineHeight: "1.6",
            whiteSpace: "pre-wrap"
        }}>
            {value || <span style={{ color: "#94a3b8" }}>No observations entered.</span>}
        </div>
    );
        ...
        <div style={{ position: "relative" }}>
            ...
            <div className="topbar">
                <div className="topbar-brand">
                    ...
                    <span style={{ color: "var(--primary)", fontWeight: 800 }}>Lab Analyst</span>
                    <span>Dashboard</span>
                </div>
                <div className="topbar-actions">
                    {isAdmin && (
                        <>
                            <button onClick={() => router.push("/admin-dashboard")} className="btn btn-ghost btn-sm">🛡️ Admin Panel</button>
                        </>
                    )}
                    <button onClick={() => { localStorage.removeItem("role"); router.push("/user-login"); }} className="btn btn-danger btn-sm">🔑 Logout</button>
                </div>
            </div>
                    ...
                    <div className="card">
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "1.5rem" }}>
                            <div>
                                ...
                                <select className="form-select" value={sampleType} onChange={e => { setSampleType(e.target.value); setSelectedSampleId(""); }}>
                            ...
                            <div>
                                ...
                                <div style={{ display: "flex", gap: "0.625rem" }}>
                                    <div style={{ flex: 1, position: "relative" }}>
                                        ...
                                        {searchSample.length > 0 && (
                                            <div style={{
                                                ...
                                                {samples.map(s => (
                                                    <div key={s.id}
                                                        onClick={() => { if (s.sample_type) setSampleType(s.sample_type); setSelectedSampleId(String(s.id)); setSearchSample(""); }}
                                                        style={{ padding: "0.75rem 1rem", cursor: "pointer", borderBottom: "1px solid #f1f5f9", fontSize: "0.875rem" }}>
                                    ...
                                    <select className="form-select" style={{ flex: 1.5 }} value={selectedSampleId} onChange={e => setSelectedSampleId(e.target.value)} disabled={!sampleType}>
                                        <option value="">Select Sample</option>
                                        {samples.map(s => <option key={s.id} value={s.id}>{s.sample_number}</option>)}
                                    </select>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="animation-fade-in space-y-8">
                            ...
                            {isSaved && (
                                <div style={{
                                    ...
                                    fontSize: "0.875rem", color: "#1e40af", fontWeight: 600
                                }}>
                                    👁️ This record has already been saved and is shown in view-only mode. Use the print buttons below to generate reports.
                                </div>
                            )}
                                ...
                                <div className="space-y-8">
                                        ...
                                        <table className="w-full text-sm">
                                            ...
                                            <tbody className="divide-y">
                                                <tr>
                                                    ...
                                                    <td className="p-1">{isSaved ? <ROInput value={form.moist_m1_i} /> : <input type="number" className="w-full p-1 border rounded" value={form.moist_m1_i} onChange={e => setForm({ ...form, moist_m1_i: Number(e.target.value) })} />}</td>
                                                    <td className="p-1">{isSaved ? <ROInput value={form.moist_m1_ii} /> : <input type="number" className="w-full p-1 border rounded" value={form.moist_m1_ii} onChange={e => setForm({ ...form, moist_m1_ii: Number(e.target.value) })} />}</td>
                                                </tr>
                                                <tr>
                                                    <td className="p-2">Mass of dish + sample (m2)</td>
                                                    <td className="p-1">{isSaved ? <ROInput value={form.moist_m2_i} /> : <input type="number" className="w-full p-1 border rounded" value={form.moist_m2_i} onChange={e => setForm({ ...form, moist_m2_i: Number(e.target.value) })} />}</td>
                                                    <td className="p-1">{isSaved ? <ROInput value={form.moist_m2_ii} /> : <input type="number" className="w-full p-1 border rounded" value={form.moist_m2_ii} onChange={e => setForm({ ...form, moist_m2_ii: Number(e.target.value) })} />}</td>
                                                </tr>
                                                ...
                                                <tr>
                                                    ...
                                                    <td className="p-1">{isSaved ? <ROInput value={form.moist_m3_i} /> : <input type="number" className="w-full p-1 border rounded" value={form.moist_m3_i} onChange={e => setForm({ ...form, moist_m3_i: Number(e.target.value) })} />}</td>
                                                    <td className="p-1">{isSaved ? <ROInput value={form.moist_m3_ii} /> : <input type="number" className="w-full p-1 border rounded" value={form.moist_m3_ii} onChange={e => setForm({ ...form, moist_m3_ii: Number(e.target.value) })} />}</td>
                                                </tr>
                                    ...
                                    <section className="bg-white p-6 rounded-xl shadow-md border border-gray-100">
                                        ...
                                        {isSaved
                                            ...
                                            : <textarea className="w-full border p-3 rounded-lg h-32 outline-none focus:ring-2 focus:ring-indigo-200" placeholder="Enter observations..." value={form.microscope_notes || ""} onChange={e => setForm({ ...form, microscope_notes: e.target.value })} />
                                        }
                                    </section>
                                </div>
                                        ...
                                        <table className="w-full text-sm">
                                            ...
                                            <tbody className="divide-y">
                                                <tr>
                                                    ...
                                                    <td className="p-1">{isSaved ? <ROInput value={form.ash_m1_i} /> : <input type="number" className="w-full p-1 border rounded" value={form.ash_m1_i} onChange={e => setForm({ ...form, ash_m1_i: Number(e.target.value) })} />}</td>
                                                    <td className="p-1">{isSaved ? <ROInput value={form.ash_m1_ii} /> : <input type="number" className="w-full p-1 border rounded" value={form.ash_m1_ii} onChange={e => setForm({ ...form, ash_m1_ii: Number(e.target.value) })} />}</td>
                                                </tr>
                                                <tr>
                                                    <td className="p-2">Mass of dish + sample (m2)</td>
                                                    <td className="p-1">{isSaved ? <ROInput value={form.ash_m2_i} /> : <input type="number" className="w-full p-1 border rounded" value={form.ash_m2_i} onChange={e => setForm({ ...form, ash_m2_i: Number(e.target.value) })} />}</td>
                                                    <td className="p-1">{isSaved ? <ROInput value={form.ash_m2_ii} /> : <input type="number" className="w-full p-1 border rounded" value={form.ash_m2_ii} onChange={e => setForm({ ...form, ash_m2_ii: Number(e.target.value) })} />}</td>
                                                </tr>
                                                <tr className="bg-gray-50 text-indigo-900 font-medium border-y">
                                                ...
                                                <tr>
                                                    ...
                                                    <td className="p-1">{isSaved ? <ROInput value={form.ash_m3_i} /> : <input type="number" className="w-full p-1 border rounded" value={form.ash_m3_i} onChange={e => setForm({ ...form, ash_m3_i: Number(e.target.value) })} />}</td>
                                                    <td className="p-1">{isSaved ? <ROInput value={form.ash_m3_ii} /> : <input type="number" className="w-full p-1 border rounded" value={form.ash_m3_ii} onChange={e => setForm({ ...form, ash_m3_ii: Number(e.target.value) })} />}</td>
                                                </tr>
                                                <tr className="bg-indigo-50 font-bold">
                                    ...
                                    <section className="bg-white p-6 rounded-xl shadow-md border border-gray-100">
                                        ...
                                        <table className="w-full text-sm">
                                            ...
                                            <tbody className="divide-y">
                                                <tr>
                                                    <td className="p-2">Mass of empty dish (m1)</td>
                                                    <td className="p-1">{isSaved ? <ROInput value={form.acid_m1_i} /> : <input type="number" className="w-full p-1 border rounded" value={form.acid_m1_i} onChange={e => setForm({ ...form, acid_m1_i: Number(e.target.value) })} />}</td>
                                                    <td className="p-1">{isSaved ? <ROInput value={form.acid_m1_ii} /> : <input type="number" className="w-full p-1 border rounded" value={form.acid_m1_ii} onChange={e => setForm({ ...form, acid_m1_ii: Number(e.target.value) })} />}</td>
                                                </tr>
                                                <tr>
                                                    <td className="p-2">Mass of acid insoluble ash + dish (m4)</td>
                                                    <td className="p-1">{isSaved ? <ROInput value={form.acid_m2_i} /> : <input type="number" className="w-full p-1 border rounded" value={form.acid_m2_i} onChange={e => setForm({ ...form, acid_m2_i: Number(e.target.value) })} />}</td>
                                                    <td className="p-1">{isSaved ? <ROInput value={form.acid_m2_ii} /> : <input type="number" className="w-full p-1 border rounded" value={form.acid_m2_ii} onChange={e => setForm({ ...form, acid_m2_ii: Number(e.target.value) })} />}</td>
                                                </tr>
                                                <tr className="bg-gray-50 text-indigo-900 font-medium border-y">
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div className="space-y-1">
                                            ...
                                            {isSaved ? <ROInput value={form.analyzed_by} /> : <input className="w-full border p-2 rounded" value={form.analyzed_by} onChange={e => setForm({ ...form, analyzed_by: e.target.value })} />}
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-xs text-gray-500 uppercase font-bold">Analyzed Date</label>
                                            {isSaved ? <ROInput value={form.analyzed_date} /> : <input type="date" className="w-full border p-2 rounded" value={form.analyzed_date} onChange={e => setForm({ ...form, analyzed_date: e.target.value })} />}
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-xs text-gray-500 uppercase font-bold">Checked By</label>
                                            {isSaved ? <ROInput value={form.checked_by} /> : <input className="w-full border p-2 rounded" value={form.checked_by} onChange={e => setForm({ ...form, checked_by: e.target.value })} />}
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-xs text-gray-500 uppercase font-bold">Checked Date</label>
                                            {isSaved ? <ROInput value={form.checked_date} /> : <input type="date" className="w-full border p-2 rounded" value={form.checked_date} onChange={e => setForm({ ...form, checked_date: e.target.value })} />}
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-4 flex items-center gap-3">
                                        {isSaved ? (
                                            <div style={{
                                                ...
                                                background: form.analysis_complete ? "#f0fdf4" : "#f9fafb",
                                                ...
                                                <span>{form.analysis_complete ? "Analysis Complete" : "Not Marked Complete"}</span>
                                            </div>
                                        ) : (
                                            <button
                                                type="button"
                                            ...
                                            >
                                                <span className="text-lg leading-none">{form.analysis_complete ? "✅" : "⬜"}</span>
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div className="space-y-1">
                                            ...
                                            {isSaved ? <ROInput value={printSettings.revisionNo} /> : <input className="w-full border p-2 rounded border-emerald-200" value={printSettings.revisionNo} onChange={e => setPrintSettings({ ...printSettings, revisionNo: e.target.value })} />}
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-xs text-gray-500 uppercase font-bold">Date of Revision</label>
                                            {isSaved ? <ROInput value={printSettings.dateOfRevision} /> : <input className="w-full border p-2 rounded border-emerald-200" placeholder="DD/MM/YYYY" value={printSettings.dateOfRevision} onChange={e => setPrintSettings({ ...printSettings, dateOfRevision: e.target.value })} />}
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-xs text-gray-500 uppercase font-bold">Issue No</label>
                                            {isSaved ? <ROInput value={printSettings.issueNo} /> : <input className="w-full border p-2 rounded border-emerald-200" value={printSettings.issueNo} onChange={e => setPrintSettings({ ...printSettings, issueNo: e.target.value })} />}
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-xs text-gray-500 uppercase font-bold">Date of Issue</label>
                                            {isSaved ? <ROInput value={printSettings.dateOfIssue} /> : <input className="w-full border p-2 rounded border-emerald-200" placeholder="DD/MM/YYYY" value={printSettings.dateOfIssue} onChange={e => setPrintSettings({ ...printSettings, dateOfIssue: e.target.value })} />}
                                        </div>
                                    </div>
                                    {!isSaved && (
                            ...
                            <div className="flex justify-end gap-3 pt-4 border-t">
                                ...
                                {!isSaved && (
                                    <button onClick={handleSave} disabled={isLoading} className="btn btn-primary btn-lg">
